...
import { ReportPaymentDialog } from "@/components/planes/report-payment-dialog";
...
export function InvoicesTable({ invoices, canReport }: { invoices: InvoiceRow[]; canReport: boolean }) {
  return (
    <Card>
      <CardHeader>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50 text-left text-xs text-muted-foreground">
                <tr>
                  ...
                  {canReport && <th className="p-2 font-medium" />}
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={invoice.id} className="border-t">
                    ...
                    {canReport && (
                      <td className="p-2 text-right">
                        {invoice.status === "pending" && <ReportPaymentDialog invoice={invoice} />}
                      </td>
                    )}
                  </tr>
                ))}
